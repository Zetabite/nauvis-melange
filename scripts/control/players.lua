    ...
    if vehicle and vehicle.valid then
        if vehicle.type == 'car' and not global.spice_effects_blacklist['name'][vehicle.name] then
            if not has_spice_effects(vehicle) then
                ...
                surface.create_entity({ name = 'nm-spice-applied-sticker', target = vehicle, position = position })
                surface.create_entity({ name = 'nm-spice-speed-sticker', target = vehicle, position = position })
                surface.create_entity({ name = 'nm-spice-vehicle-regen-sticker', target = vehicle, position = position })
            end
        end
    end
end
...
local manage = {
    radar = {
        enable = function(player_index)
            ...
            local reference = surface.create_entity({name = 'nm-spice-radar', position = position, force = character.force})
...
consume_spice = function(player_index, factor, consequence, pre_consumed)
    ...
    if needed > 0 then
        local item = {name = 'nm-spice', count = needed}
        if consequence then
            local actual = inventory.get_item_count('nm-spice')
            if actual < needed then
                ...
                elseif consequence == 'no_consumption' then
                    if pre_consumed > 0 then
                        item = {name = 'nm-spice', count = pre_consumed}
...
apply_spice_to_player = function(player)
    ...
    if not (kux_running_setting and kux_running_setting.value) then
        surface.create_entity({ name = 'nm-spice-speed-sticker', target = character, position = position })
    end
    surface.create_entity({ name = 'nm-spice-regen-sticker', target = character, position = position })
    -- Table changes
    global.render_table.spice_overlay[player_index] = game.players[player_index].mod_settings['nm-spice-overlay'].value
...
player_joined = function(event)
    ...
    if has_spice_influence(player_index) and player.character and player.character.valid then
        global.render_table.spice_overlay[player_index] = game.players[player_index].mod_settings['nm-spice-overlay'].value
    end
end
...
player_moved = function(event)
    ...
    if character and character.valid then
        if has_spice_influence(player_index) then
            local radar = global.players[player_index].radar
            local surface = character.surface
            local reference = surface.create_entity({name = 'nm-spice-radar', position = character.position, force = character.force})
            if radar.reference and radar.reference.valid then
                radar.reference.destroy()
            end
            radar.reference = reference
